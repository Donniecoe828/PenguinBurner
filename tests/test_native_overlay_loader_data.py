...
def test_overlay_command_buffers_get_loader_data(tmp_path: Path) -> None:
    """Every command buffer the overlay allocates is stamped by the loader.

    A command buffer is dispatchable, so a layer above ours reads its dispatch
    table from the handle itself. Skipping the loader's callback leaves the
    table the layer below us wrote, and the validation layer aborts the game
    with "The VkDevice dispatch handle was not found".
    """
    binary = _build_loader_data_probe(tmp_path)
    ...
    result = subprocess.run(
        ...
        text=True,
        capture_output=True,
        env={"PATH": os.environ.get("PATH", ""), "PB_OVERLAY": "1"},
    )

    assert result.stdout.strip() == "ready=1 allocated=3 stamped=3"
...
VKAPI_ATTR VkResult VKAPI_CALL set_loader_data(VkDevice, void*) {
    ++g_stamped;
    return VK_SUCCESS;
}
...
VKAPI_ATTR void VKAPI_CALL destroy_object(
    VkDevice, Handle, const VkAllocationCallbacks*) {}
...
pblayer::DeviceContext make_device_context() {
    pblayer::DeviceContext context{};
    context.device = fake_handle<VkDevice>();
    context.set_device_loader_data = set_loader_data;
...
int main() {
    pblayer::DeviceContext device_context = make_device_context();
